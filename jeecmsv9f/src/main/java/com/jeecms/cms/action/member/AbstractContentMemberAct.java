...
public class AbstractContentMemberAct {
	...
	public String add(boolean hasPermission,String nextUrl,HttpServletRequest request,HttpServletResponse response, ModelMap model) {
		...
		if(hasPermission){
			...
			errors.addErrorCode("error.uploadMoreNumber", user.getGroup());
	...
	public String save(String title, String author, String description,
			...
			Double[] rewardFix,String nextUrl,String typeImg,
		...
		WebErrors errors = validateSave(title, author, description, txt,tagStr,
				channelId, site, user, captcha, request, response);
		if (errors.hasErrors()) {
			return FrontUtils.showError(request, response, model, errors);
		}
		...
		c = contentMng.save(c, ext, t,null, null, null, tagArr, attachmentPaths,
				attachmentNames,attachmentFilenames, picPaths
				,picDescs,channelId,typeId, null, true,charge,
	...
	protected String update(Integer id, String title, String author,
		...
		c=contentMng.update(c, ext, t,tagArr, null, null, null, attachmentPaths, 
				attachmentNames,attachmentFilenames, picPaths
				,picDescs,null, channelId, null, null, charge, 
				chargeAmount,rewardPattern,
