...
public class CmsVoteTopicDaoImpl extends
	...
	public Pagination getPage(Integer siteId,Short statu, int pageNo, int pageSize) {
		...
		if (statu!=null) {
			...
			//待开始
			case 1:
				f.append(" and bean.startTime>=:nowTime").setParam("nowTime", new Date());
				break;
			//进行中
			case 2:
				f.append(" and ((bean.startTime<:nowTime or bean.startTime is null) and (bean.endTime>=:nowTime or bean.endTime is null))").setParam("nowTime", new Date());
				break;
			//已结束
			case 3:
				f.append(" and bean.endTime<:nowTime").setParam("nowTime", new Date());
				break;
			default:
				break;
			}
